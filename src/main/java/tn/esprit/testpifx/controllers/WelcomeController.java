...
public class WelcomeController {
    ...
    private void initialize() {
        ...
        // Apply full-screen mode as soon as the controller is initialized
        javafx.application.Platform.runLater(() -> {
            if (welcomeLabel != null && welcomeLabel.getScene() != null && 
                welcomeLabel.getScene().getWindow() != null) {
                Stage stage = (Stage) welcomeLabel.getScene().getWindow();
                Utils.setFullScreenMode(stage);
                System.out.println("Applied full-screen mode to welcome screen");
            }
            
            // Initialize charts with placeholder data to ensure they render properly
            initializeCharts();
        });
    ...
    public void setCurrentUser(User user) {
        ...
        if (user != null && welcomeLabel != null) {
            ...
            System.out.println("WelcomeController: Current user set to " + user.getUsername());
            
            // Check if user has only USER role (no ADMIN or MANAGER role)
        ...
        javafx.application.Platform.runLater(() -> {
            // Wait a small moment before trying to update statistics
    ...
    private void tryUpdateStatistics() {
        if (userService != null && teamService != null && currentUser != null) {
            ...
            // Use Platform.runLater to ensure UI components are fully loaded 
            javafx.application.Platform.runLater(() -> {
                updateStatistics();
            });
        } else {
    ...
    private void populateTeamSizeChart(List<tn.esprit.testpifx.models.Team> teams) {
        ...
        // Add series to chart
        teamSizeChart.getData().addAll(deptSeries, projectSeries);
    ...
    private void populateActivityChart() {
        ...
        activityLineChart.getData().addAll(loginSeries, registrationSeries);
    ...
    private void populateActivityTable() {
        ...
        try {
            // Explicitly cast the table and columns to the proper types
            TableView<ActivityData> table = (TableView<ActivityData>) activityTable;
            TableColumn<ActivityData, String> typeCol = (TableColumn<ActivityData, String>) activityTypeColumn;
            TableColumn<ActivityData, String> userCol = (TableColumn<ActivityData, String>) activityUserColumn;
            TableColumn<ActivityData, String> descCol = (TableColumn<ActivityData, String>) activityDescriptionColumn;
            TableColumn<ActivityData, String> timeCol = (TableColumn<ActivityData, String>) activityTimeColumn;
